...
public class NuevaCompra extends JFrame {
	...
	private void cargar() {
	    ...
	    this.dtm.setColumnIdentifiers(new Object[] { "Cantidad", "Producto y detalles", "Precio indiv.", "Sub Total", "Lote" });
	  ...
	  public void cargarBuscador() {
	    ...
	    try {
	      ...
	      while (this.rs.next())
	        this.ac.addItem(this.rs.getString("producto") + " " + this.rs.getString("detalles") + " " + this.rs.getString("marca") + " " + this.rs.getString("color") + "*" + this.rs.getString("laboratorio") + " * " + this.rs.getString("unimedida") + 
	  ...
	  protected void keyTypedTxtDescripcion(KeyEvent e) { char c = e.getKeyChar();
	  ...
	  protected void actionPerformedBtnCrearProducto(ActionEvent arg0) {
	    try {
	      if (this.tbCompras.getRowCount() == 0) {
	        ...
	        int opc = JOptionPane.showConfirmDialog(null, "¿Está seguro de registrar la compra?\nEste registro no se podrá eliminar o modificar en un futuro, le pedimos por favor verifique sus productos y precios", "Confirmar", 0, 3);
	        if (opc == 0)
	        {
	          int tipComprobante = 0; tipComprobante = this.cbTipoComprobante.getSelectedIndex();
	          String serie = ""; serie = this.txtSerie.getText();
	          String nroSerie = ""; nroSerie = this.txtNroSerie.getText();
	          int idDistrib = 0; idDistrib = ((Distribuidores)this.cbDistribuidor.getItemAt(this.cbDistribuidor.getSelectedIndex())).getIddist();
	          String moneda = ""; moneda = this.cbMoneda.getSelectedItem().toString();
	          String tc = ""; tc = this.txtTipoCambio.getText();
	          Object fechaEmision = null;
	          Object fechaVencimiento = null;

	          String nota = ""; nota = this.txtNota.getText();
	          String metPago = ""; this.cbMetPago.getSelectedItem().toString();
	          double total = 0.0D;
	          ...
	          this.consulta.iniciar();
	          this.consulta.registrarCompra(tipComprobante, serie, nroSerie, idDistrib, moneda, tc, nota, metPago, fechaEmision, fechaVencimiento, this.idUsuario, total, pagado, saldo);
	          this.consulta.reset();
	          ...
	          for (int i = 0; i < this.tbCompras.getRowCount(); i++) {
	            ...
	            this.consulta.iniciar();
	            this.consulta.registrarCompraDetalles(idCompra, idProd, cantProd, preIndivProd, preSubTotProd, lote);
	  ...
	  protected void focusGainedTxtNombreProducto(FocusEvent e) { seleccionarTexto(e); }
	  ...
	  public void AgregarProductoATabla() {
	    try {
	      ...
	      this.dtm.addRow(new Object[] { Double.valueOf(cantidad), nomProducto, Double.valueOf(precioUnidad), Double.valueOf(precioSubTot), lote });
	  ...
	  private void sumarTotal() { double total = 0.0D;
	    for (int i = 0; i < this.tbCompras.getRowCount(); i++) {
	      double cantProd = Float.parseFloat(this.tbCompras.getValueAt(i, 0).toString());
