...
public class ModificarPrecioVenta2 extends JFrame implements ActionListener, WindowListener, KeyListener, ItemListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModificarPrecioVenta2 frame = new ModificarPrecioVenta2(null, 0, null, null, 0.0D, 0.0D, 0.0D, 0.0D,
							0.0D);
	...
	public ModificarPrecioVenta2(Ventas ventas, int idProd, String nomProdVenta, String uniMedVenta, double cantVenta,
			double preCDesC, double subTotVenta, double preCompraVenta, double descTVenta) {
		...
		Image imgLogo = new ImageIcon(this.getClass().getResource("/imgbasura.png")).getImage().getScaledInstance(100,
	...
	protected void actionPerformedBtnCambiar(ActionEvent arg0) {
		try {
			...
			if (newCant <= 0.0 || newPrecio < 0.0 || total < 0.0) {
				...
				// this.ventas.actualizartabla(newCant, newPrecio, preCompra * newCant, total,
				// descTot, newUnimed, this.uniMedVenta);
				this.ventas.actualizartabla(newCant, newPrecio, preCompra * newCant, total, descTot, newUnimed,
						this.uniMedVenta);
				dispose();
			}
		} catch (Exception e) {
